...
## ⚠️ پیش از کامپایل: فایل `miniaudio.h`

طبق صورت پروژه، فایل `miniaudio.h` توسط استاد ارائه می‌شود و **نباید تغییر کند**.
این فایل عمداً داخل پکیج قرار داده **نشده** است. کافی است نسخه‌ی اصلی
`miniaudio.h` را داخل پوشه‌ی `src/` کپی کنید:
